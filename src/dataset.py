...
class NPYDatasetSC_AMP_CNN(Dataset):
    """CNN version simply adds a channel dimension. """
...
class NPYDatasetCL_CSLD(Dataset):
    """CNN version simply adds a channel dimension. """
    def __init__(self, csv_path, global_mean=1.0, manipulant_select=None):
        """
        Args:
        ...
        # NOTE: new feture 20260203 to select only specific consonant data.
        if manipulant_select is not None:
            df = df[df['consonant'].isin(manipulant_select)].reset_index(drop=True)
...
class NPYDatasetRC_CSLD(Dataset):
    """CNN version simply adds a channel dimension. """
    def __init__(self, csv_path, global_mean=1.0, manipulant_select=None):
        """
        Args:
...
class NPYDatasetInfoPredict_AMP(Dataset):
    """CNN version simply adds a channel dimension. """
    def __init__(self, csv_path, global_mean=1.0, manipulant_select=None, manipulant_label=None):
        """
        Args:
            csv_path (str): Path to the CSV metadata file.
            base_path (str): Base directory where .npy files are stored.
        ...
        # unique_labels = ["asa", "aca", "atsa", "atca", "asha", "acha", "atsha", "atcha"]
        unique_labels = manipulant_label
        self.label_to_index = {lab: i for i, lab in enumerate(unique_labels)}
        self.index_to_label = {i: lab for lab, i in self.label_to_index.items()}
...
class NPYDatasetInfoCollect_AMP(Dataset): 
    """
    Dataset class that returns data along with metadata information.
    For EVLUATION only. 
    """
...
class NPYDatasetInfoCollect_CSLD(Dataset): 
    """
    Dataset class that returns data along with metadata information.
    For EVLUATION only. 
    """
