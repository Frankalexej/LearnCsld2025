    ...
    for run_name, run_name_short in zip(run_names, run_names_short):
        ...
        for run_time in range(config.RUN_TIMES_START, config.RUN_TIMES_END): 
            for epoch in range(config.EPOCH_START, config.EPOCH_END, config.EPOCH_INTERVAL):
                    ...
                    for i in range(C):
                        for j in range(i, C):
                            ...
                            if c1 == c2:
                                ...
                                # cluster centroids
                                mean_A = vec_v[idxA].mean(axis=0)
                                mean_B = vec_v[idxB].mean(axis=0)

                                # Euclidean distance between cluster means
                                cluster_mean_dist = np.linalg.norm(mean_A - mean_B)

                                # Cosine distance between cluster means
                                mean_A_norm = mean_A / (np.linalg.norm(mean_A) + 1e-9)
                                mean_B_norm = mean_B / (np.linalg.norm(mean_B) + 1e-9)
                                cluster_mean_cos_dist = 1 - np.dot(mean_A_norm, mean_B_norm)

                                # D_sub = D_v[np.ix_(sub_idx, sub_idx)]
                                score = cluster_mean_dist
                                cos_score = cluster_mean_cos_dist
                            ...
                            # -------------------------------
                                ...
                                # cluster centroids
                                mean_A = vec_v[idx1].mean(axis=0)
                                mean_B = vec_v[idx2].mean(axis=0)
                ...
                sil_df.to_csv(
                    f"{save_path}/distance_E{epoch}_R{run_time}.csv",
                    index=False
                )
