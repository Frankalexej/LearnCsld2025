...
from dataset import NPYDatasetCL_CNN as ThisCLDataset
from dataset import NPYDatasetRC_CNN as ThisRCDataset
import wandb
import numpy as np
import sys
...
def main(config_path, run_time=0):
    ...
    testloader1 = DataLoader(testset1, batch_size=config.BATCH_SIZE, shuffle=True)
    testloader2 = DataLoader(testset2, batch_size=config.BATCH_SIZE, shuffle=True)
    ...
    if checkpoint_files:
        ...
        sample_test_wrapper(sample_list,model1,config.DEVICE,similarity_config,0)   
    ...
    if start_epoch <= config.PRE_EPOCHS :
        ...
        for epoch in range(start_epoch, first_end_epoch):
            ...
            #test some samples
            sample_test_wrapper(sample_list,model1,config.DEVICE,similarity_config,epoch+1)
    ...
    if second_start_epoch < start_epoch+config.PRE_EPOCHS+config.POST_EPOCHS:
        ...
        for epoch in range(second_start_epoch, start_epoch+config.PRE_EPOCHS+config.POST_EPOCHS):
            ...
            #test some samples
            sample_test_wrapper(sample_list,model2,config.DEVICE,similarity_config,epoch+1)
    wandb.finish()
