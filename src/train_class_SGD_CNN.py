...
from dataset import NPYDatasetCL_CNN as ThisDataset
import wandb
import numpy as np
import sys
...
def main(config_path, run_time=0):
    ...
    # Load dataset
    dataset1 = ThisDataset(
        csv_path=config.CSV_PATH
    )
    dataset2 = ThisDataset(
    ...
    model = SimpleResNet1D(hid_features=config.HID_FEATURES, out_features=config.OUT_FEATURES).to(config.DEVICE)
    similarity_config = config.SIMILARITY
    criterion = torch.nn.CrossEntropyLoss(reduction="mean") # Using CE for classification, better performing than MSE. 
    # optimizer = optim.SGD(
    #     model.parameters(),
    #     lr=config.LR,
    #     momentum=0.0,
