...
import tqdm
from model import NonLinearFCClass as ThisPredictor
from torch.utils.data import DataLoader
from dataset import NPYDatasetInfoPredict_AMP as CollectDataset
import numpy as np
import sys
...
def evaluate_predict_outputs(
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm.tqdm(data_loader)):
        ...
        # forward pass -> vectors
        preds = model.predict(inputs, mapper=mapper)  # expected [B]

        # append metadata DataFrame
        batch_df = batch_meta_to_df(targets)
        # add predictions column
        batch_df["predicted"] = preds
        csv_frames.append(batch_df)

    if not csv_frames:
        raise RuntimeError("No data collected. Check your loader and input_key.")
...
def main(config_path, run_name=None, write_run_name=None, run_time=0):
    ...
    eval_save_dir_hyper = os.path.join(config.WRITE_BASE_PATH, 'pred_outputs', config.WRITE_RUN_NAME, f'{run_time}')
    ...
    L1_manipulant_label = config.L1_MANIPULANT_LABEL
    L2_manipulant_label = config.L2_MANIPULANT_LABEL
    ...
    if checkpoint_files:
        for check_point_file in checkpoint_files: 
            ...
            if this_epoch <= config.PRE_EPOCHS: 
                print(f"Epoch {this_epoch} in Pre, not included. ")
                continue
            this_checkpoint = os.path.join(save_dir, check_point_file)
            print(f"Loading checkpoint from {this_checkpoint}")

            model = ThisPredictor(in_features=config.IN_FEATURES, 
                    hid_features=config.HID_FEATURES, 
                    out_features=config.OUT_FEATURES_2).to(config.DEVICE)
            model.load_state_dict(torch.load(this_checkpoint), strict=False)
            eval_save_dir = eval_save_dir_hyper
            os.makedirs(eval_save_dir, exist_ok=True)
            evaluate_predict_outputs(
                model=model,
                data_loader=dataloader_L2,
                device=config.DEVICE,
                csv_path=os.path.join(eval_save_dir, f"pred_E{this_epoch}_L2.csv"),
                mapper=dataset_L2.index_to_label, 
                to_float32=True
            )
    else:
