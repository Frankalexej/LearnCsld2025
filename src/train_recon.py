...
import tqdm
from model import SimpleNonlinearFCAutoencoder
...
def main(config_path, run_time=0):
    ...
    model = SimpleNonlinearFCAutoencoder(in_features=config.IN_FEATURES, hid_features=config.OUT_FEATURES, out_features=config.IN_FEATURES).to(config.DEVICE)
    similarity_config = config.SIMILARITY
    criterion = SimpleMSE(reduction="mean")
    # optimizer = optim.SGD(
    #     model.parameters(),
    #     lr=config.LR,
    #     momentum=0.0,
