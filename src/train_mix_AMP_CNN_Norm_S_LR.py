...
import tqdm
from model import TanhFCRecon as ThisRCModel    # NOTE: we do not yet run these using CNN! 
from model import TanhFCClass as ThisCLModel
from model import SimpleResNet1Dv2 as ThisSCModel
from torch.utils.data import DataLoader
from dataset import NPYDatasetCL_AMP as ThisCLDataset
from dataset import NPYDatasetRC_AMP as ThisRCDataset
from dataset import NPYDatasetSC_AMP_CNN as ThisSCDataset
import wandb
import numpy as np
import pandas as pd
import sys
...
def main(config_path, run_time=0, this_seed=0):
    ...
    if pre_method == "RC": 
        ...
        dataset1 = ThisSCDataset(
            csv_path=config.CSV_PATH, 
            global_mean=global_mean, 
            manipulant_select=L1_manipulant_select
        )
        testset1 = ThisSCDataset(
            csv_path=config.CSV_PATH4, 
            global_mean=global_mean, 
            manipulant_select=L1_manipulant_select
        )
        model1 = ThisSCModel(out_features=hid_features).to(config.DEVICE)
    ...
    if post_method == "RC": 
        ...
        # contrastive learning, supcon
        supcon_temperature = getattr(config, "SUPCON_TEMPERATURE", 0.07)
        dataset2 = ThisSCDataset(
            csv_path=config.CSV_PATH2, 
            global_mean=global_mean, 
            manipulant_select=L2_manipulant_select
        )
        testset2 = ThisSCDataset(
            csv_path=config.CSV_PATH3, 
            global_mean=global_mean, 
            manipulant_select=L2_manipulant_select
        )
        model2 = ThisSCModel(out_features=hid_features).to(config.DEVICE)
    ...
    if start_epoch <= config.PRE_EPOCHS + 1: # +1 allows the case of stopping between after 100 and before 110
        ...
        if freeze_for_L2: 
            ...
            model2.set_freeze(freeze_encoder=True)
