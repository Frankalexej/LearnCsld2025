...
import tqdm
from model import NonLinearFCReconEncoderConsolidation as ThisRCModel
from model import NonLinearFCClass as ThisCLModel
from torch.utils.data import DataLoader
from dataset import NPYDatasetCL_CSLD as ThisCLDataset
from dataset import NPYDatasetRC_CSLD as ThisRCDataset
import wandb
import numpy as np
import pandas as pd
import sys
...
def main(config_path, run_time=0, this_seed=0):
    ...
    if start_epoch <= config.PRE_EPOCHS + 1: # +1 allows the case of stopping between after 100 and before 110
        ...
        if pre_method != post_method: 
            ...
            print(f"{pre_method} -> {post_method} with same output features. Loading whole model. ")
