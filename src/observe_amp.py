    ...
    for run_name, run_name_short in zip(run_names, run_names_short):
        ...
        for run_time in range(config.RUN_TIMES_START, config.RUN_TIMES_END): 
            for epoch in range(config.EPOCH_START, config.EPOCH_END, config.EPOCH_INTERVAL):
                ...
                # -----------------------------------------------
                vowels = metas["consonant"].unique()
                sil_results = []

                consonant_list = metas["vowel"].unique().tolist()

                rng = np.random.default_rng(12345)  # stable randomness

                for vowel in vowels:

                    mask_vowel = metas["consonant"] == vowel
                    ...
                    consonants_here = meta_v["vowel"].unique().tolist()
                    ...
                    for i in range(C):
                        for j in range(i, C):
                            ...
                            idx_c1 = meta_v.index[meta_v["vowel"] == c1].to_numpy()
                            idx_c2 = meta_v.index[meta_v["vowel"] == c2].to_numpy()
